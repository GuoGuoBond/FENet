...
class KittiDataset(Custom3DDataset):
    ...
    def show(self, results, out_dir, show=True, pipeline=None):
        ...
        for i, result in enumerate(results):
            ...
            gt_labels = torch.from_numpy(self.get_ann_info(i)['gt_labels_3d'])
            cls_mask = (gt_labels >= 0) & (gt_labels <= 2)
            gt_bboxes = gt_bboxes[cls_mask].numpy() # 训练时只使用三类的gtbox作为标签，所以可视化时排除其他类
