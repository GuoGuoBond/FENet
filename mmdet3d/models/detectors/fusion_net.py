...
class Fusion_Net(Base3DDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(Fusion_Net, self).__init__(init_cfg=init_cfg)
    ...
    def extract_pts_feat(self, points_cat, img_feats, img_metas):
        """Extract features of points."""
        if not self.with_pts_bbox:
            return None
        x = self.pts_backbone(points_cat)
        if self.with_pts_neck:
            x = self.pts_neck(x)
        return x

    def extract_feat(self, points_cat, img, img_metas):
        """Extract features from images and points."""
        img_feats = self.extract_img_feat(img, img_metas)
        pts_feats = self.extract_pts_feat(points_cat, img_feats, img_metas)
    ...
    def forward_train(self,
        ...
        img_feats, pts_feats = self.extract_feat(
            points_cat, img=img, img_metas=img_metas)
        losses = dict()
        if pts_feats:
    ...
    def forward_pts_train(self, pts_feats, points, gt_bboxes_3d, gt_labels_3d, pts_semantic_mask,
                       pts_instance_mask, img_metas, gt_bboxes_ignore=None):
        ...
        """
        bbox_preds = self.pts_bbox_head(pts_feats, self.train_cfg.pts.sample_mod, self.train_cfg.pts.stage)
    ...
    def simple_test_pts(self, x, points_cat, img_metas, rescale=False):
        """Test function of point cloud branch."""
        bbox_preds = self.pts_bbox_head(x, self.test_cfg.pts.sample_mod)
        bbox_list = self.pts_bbox_head.get_bboxes(
            points_cat, bbox_preds, img_metas, rescale=rescale) # 在这一步对结果进行nms
        bbox_results = [
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False):
        ...
        if pts_feats and self.with_pts_bbox:
            bbox_pts = self.simple_test_pts(
                x=pts_feats, points_cat=points_cat, img_metas=img_metas, rescale=rescale)
    ...
    def show_results(self, data, result, out_dir, show, score_thr):
        ...
        for batch_id in range(len(result)):
            ...
            show_result(points=points, gt_bboxes=None, pred_bboxes=pred_bboxes,
                        out_dir=out_dir, filename=file_name, show=show)