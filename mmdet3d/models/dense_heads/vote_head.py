...
from mmcv.ops import furthest_point_sample, QueryAndGroup, gather_points
...
from mmdet3d.models.model_utils import PrototypicalVoteModule
from mmdet3d.models.model_utils import Confidence_mlps
from mmdet3d.models.model_utils import TransformerLayer
...
class VoteHead(BaseModule):
    ...
    def __init__(self,
        ...
        self.num_points = vote_module_cfg.num_points
        for i in range(len(confidence_mlps)):
            self.confidence_mlps = Confidence_mlps(in_channels, confidence_mlps[i], num_classes)

        self.prototypical_transformer = TransformerLayer(**transformer_cfg)
        pass
    ...
    def forward(self, feat_dict, sample_mod, stage='test'):
        """Forward pass.
        ...
        results = dict()
        seed_points, seed_features, seed_indices, cls_points, cls_features, cls_indices, \
            input_points, input_features, input_indices = self._extract_input(feat_dict) # backbone的输出

        if self.confidence_mlps != None:
            cls_feature, class_pred = self.confidence_mlps(seed_features)
            cls_points.append(seed_points)
            cls_features.append(cls_feature)  # (B, N, Class)
            cls_indices.append(seed_indices)
            origin_vote_points, origin_vote_features, sample_idx = \
                self.confidence_mlps.get_topk(seed_points, seed_features, seed_indices, self.num_points)
        else:
            origin_vote_points = seed_points
            origin_vote_features = seed_features

        results['input_points'] = input_points
        results['input_features'] = input_features
        results['input_indices'] = input_indices
        results['cls_points'] = cls_points
        results['cls_features'] = cls_features
        results['cls_indices'] = cls_indices
        results['origin_vote_points'] = origin_vote_points

        # 1. generate vote_points from seed_points
        transformer_seed_features = self.prototypical_transformer(tgt=seed_features, class_pred=class_pred, stage=stage)[0]
        origin_vote_features = gather_points(transformer_seed_features, sample_idx)
        vote_points, vote_features, vote_offset = self.vote_module(
            origin_vote_points, origin_vote_features)

        results['seed_points'] = seed_points
        results['seed_indices'] = seed_indices
        results['vote_points'] = vote_points
        results['vote_features'] = vote_features
        results['vote_offset'] = vote_offset
        ...
        if sample_mod == 'vote':
            ...
            aggregation_inputs = dict(
                ...
                features=transformer_seed_features,
                target_xyz=vote_points)
        else:
            raise NotImplementedError(
                f'Sample mode {sample_mod} is not supported!')

        vote_aggregation_ret = self.vote_aggregation(**aggregation_inputs) #单层pointnet sa
        ...
        # 3. predict bbox and score fixme con1d vs linear
        cls_predictions, reg_predictions = self.conv_pred(aggregated_features)

        # 4. decode predictions
        """
        cls_predictions (B, Class, N)
        reg_predictions (B, center_offset(3)+size(3)+dir_class(num_dir_bins)+dir_res_norm(num_dir_bins), N)
        aggregated_points (B, N, 3)
        
        2pai/360度 = 弧度/角度 （这里改为2pai/num_dir_bins = 弧度/预测值）
        """
