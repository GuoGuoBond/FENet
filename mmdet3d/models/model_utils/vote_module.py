...
class VoteModule(nn.Module):
    ...
    def forward(self, seed_points, seed_feats):
        ...
        batch_size, feat_channels, num_seed = seed_feats.shape  # 输入特征的维度
        num_vote = num_seed * self.vote_per_seed  # 每个点投票多少次
        ...
        offset = votes[:, :, :, 0:3]  # 投票生成的offset
        if self.vote_xyz_range is not None:  # 过滤掉超出限定范围的offset
            limited_offset_list = []
        ...
        return vote_points, vote_feats, offset  # 偏移后的点  使用的特征  特征得到的偏移量
