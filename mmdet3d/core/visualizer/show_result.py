...
def show_result(points,
    ...
    if pred_bboxes is not None:
        ...
        Pedestrian_mask = (pred_labels == 0)
        Cyclist_mask = (pred_labels == 1)
        Car_mask = (pred_labels == 2)

        _write_oriented_bbox(pred_bboxes.numpy(),
                             osp.join(result_path, f'{filename}_pred.obj'))

        _write_oriented_bbox(pred_bboxes[Car_mask, :].numpy(),
                             osp.join(result_path, f'{filename}_pred_Car.obj'))

        _write_oriented_bbox(pred_bboxes[Cyclist_mask, :].numpy(),
                             osp.join(result_path, f'{filename}_pred_Cyclist.obj'))

        _write_oriented_bbox(pred_bboxes[Pedestrian_mask, :].numpy(),
                             osp.join(result_path, f'{filename}_pred_Pedestrian.obj'))
